...
Only that line is read, so naming other filenames in the prose (an env file, a config, a spec link) is harmless. Each cite resolves to a real file by matching the tail of its path against the tree, so cite a file however you like — a bare `fileset.ts` and a full `src/fileset.ts` both find the one file. When a bare filename is genuinely ambiguous — the tree holds several files of that name and the cite carries nothing to narrow it — the ticket stays confident and that cite simply collides with any file of that name (it cannot be pinned to one). A ticket with no marker line falls back to a whole-body scan and is far likelier to resolve as under-specified — so the planner halts on it; the marker line is what makes a ticket schedulable AFK. **The one exception:** a selection that resolves to a single issue has no co-wave to collide with, so the file-set check is *skipped* as vacuous — `campaign <id>` runs a lone issue with no marker line (the provenance names the skipped check). The marker still earns its keep the moment the ticket rides in a real wave beside others.
