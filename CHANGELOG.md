...
- [user] On phones (under 640px), the live-tail header drops the redundant agent summary — it duplicates the agent dropdown beside it — reclaiming that width in every pane sharing the header row. This does not yet fit the header at phone widths: at ~440px the title still wraps and the right-hand controls are still clipped, so #336 stays open (#336).

**Bug fixes:**
- [ops] `changelog collect` no longer reorders a consuming project's existing changelog sections. The fold re-renders the milestone it folds into, and it knew only Vetinari's own five section labels, so a project using more (the sibling jjforge's `Security`, `Infrastructure`, `Architecture`, `Testing`, `Code quality`) had them shunted below `Documentation` on any collect — silently, since a repeated-label lint cannot detect a reordering. The fold now knows the full set.
