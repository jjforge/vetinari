...
**Every change that adds or alters a command, flag, behaviour, config surface, or output gets a changelog entry**, citing the issue — in the same change that lands it, not a later pass. A purely internal refactor with no user-visible effect needs none. The format is dated milestones, newest first, with audience-tagged bullets (`[user]`/`[ops]`/`[api]`/`[internal]`) under bold section labels — the full convention (tags, labels, which milestone to add to) is [`docs/changelog-conventions.md`](docs/changelog-conventions.md).

**How the entry is written depends on who is writing it.** Every ticket would otherwise touch [`CHANGELOG.md`](CHANGELOG.md), so co-wave campaign branches conflict on it and halt the campaign. So a **campaign agent writes a fragment** to `changelog.d/<task-id>.md` (a `section:` label plus tagged bullets — the format lives in `docs/changelog-conventions.md`) and never edits `CHANGELOG.md`; the orchestrator folds a wave's fragments into `CHANGELOG.md` in one commit at merge. **Interactive work** — a human not racing a wave — may instead edit `CHANGELOG.md` directly, or write a fragment and run `vetinari changelog collect`. This is enforced on the implementing agent via `prompts/tdd.md` (the TDD prompt every campaign drives), not in `to-tickets`/`/implement` — those are external skills we do not own, so the rule lives where we control it: this file and the prompt.
