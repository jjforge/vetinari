...
/**
 * `POST /resume` — the wave-park recovery action. When a wave gated its merged
 * base red the campaign pauses for a human (ADR 0013); once they have fixed
 * forward, this shells `campaign --resume` in the selected project's own root
...
export const handleResume: RouteHandler = async (req, res, url, deps) => {
  ...
  deps.spawn(process.execPath, [...process.execArgv, process.argv[1], "campaign", "--resume"], {
