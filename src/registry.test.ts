...
test("readProject returns undefined for a pointer whose base location is gone", () => {
  const read = readProject(pointer({ baseLocation: join(tmpdir(), `vetinari-gone-${Date.now()}-${counter++}`) }));
...
test("readProjects skips a stale pointer and never throws on it", () => {
  ...
  register(configDir, pointer({ project: "stale", baseLocation: join(tmpdir(), `vetinari-gone-${Date.now()}-${counter++}`) }));
...
test("autoRegister writes the current project's pointer and is idempotent on re-run", () => {
  ...
  assert.deepEqual(listProjects(configDir), [
    { project: "jjforge", projectRoot: "/home/me/code/jjforge", baseLocation: "/home/me/code/jjforge/.vetinari.local" },
  ]);
});
