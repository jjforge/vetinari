...
test("the aggregated prune-preview and graft-rejection pages draw from the shared palette, never a hand-authored hex (Appendix A, #317)", () => {
  ...
  // The prune preview is a risky confirmation, so its card takes the 1px-outline confirmation
  // treatment in the risky-action coral — no coloured left edge (#328). Appendix A reserves the
  // 3px left edge for amber ("needs you") alone, which a control's confirm card is not.
  assert.match(prunePage, /\.card \{[^}]*border: 1px solid var\(--color-red\)/);
  assert.doesNotMatch(prunePage, /\.card \{[^}]*border-left: 3px/);
  // The graft-rejection card is a "fix these" needs-you surface, so it keeps the amber 3px edge.
  assert.match(graftPage, /\.card \{[^}]*border-left: 3px solid var\(--color-yellow\)/);
});
