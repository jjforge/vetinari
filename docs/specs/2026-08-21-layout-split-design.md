...
I want to run vetinari against several of my projects at once, but the way a
...
- **`vetinari/`** — committed. The project's versioned configuration: the config
  module, the project's `Dockerfile`, and any prompt override.
- **`.vetinari.local/`** — excluded (gitignored). This machine's moving parts:
  ...
  (`settings.local.json`, `.env.local`). vetinari itself is a shared machine
...
canonical config location is `vetinari/config.mts`; the old `.sandcastle/config.mts`
location keeps working but emits a deprecation warning, so a live setup is
never broken the moment it upgrades.

A `vetinari migrate` command moves an existing project onto the new layout in one
step, so I never have to hand-shuffle directories or hand-edit `.gitignore`.

## User Stories

1. As a maintainer, I want my project's Vetinari configuration to live in a
   committed `vetinari/` directory, so that it is versioned and shared with
   everyone who checks out the repo.
2. As a maintainer, I want all credentials, logs, and state under a single excluded
   `.vetinari.local/` directory, so that nothing machine-local is ever committed.
...
4. As a maintainer, I want `vetinari/config.mts` to be the canonical config
...
7. As a maintainer, I want the deprecation warning to name the new location, so
   that I know exactly where to move the file.
7. As a maintainer with no config anywhere, I want a clear error that lists the
   canonical location and how to create one, so that I am not left guessing.
7. As a maintainer, I want run state (`logs/`, `parked/`) to default into
    `.vetinari.local/` instead of `.sandcastle/`, so that state follows the new
    excluded directory with no extra configuration.
7. As a maintainer, I want a fresh checkout to gitignore `.vetinari.local/`, so
    that secrets and state cannot be accidentally committed.
7. As a maintainer, I want to keep `.sandcastle/` ignored during the transition,
    so that a not-yet-migrated project does not start leaking its old state.
7. As a maintainer, I want a `vetinari migrate` command that moves an existing
    project onto the new layout, so that I do not have to shuffle directories by
    hand.
7. As a maintainer, I want `migrate --dry-run` to print exactly what it would move
    without touching anything, so that I can review the plan before committing to it.
7. As a maintainer, I want `migrate` to move my config from its old location into
    `vetinari/`, so that it becomes committed and canonical.
7. As a maintainer, I want `migrate` to move my old `.sandcastle/` state and
    secrets into `.vetinari.local/`, so that nothing is lost and the old
    directory is retired.
7. As a maintainer, I want `migrate` to update `.gitignore` to exclude
    `.vetinari.local/`, so that the new excluded directory is protected without my
    editing the file.
7. As a maintainer, I want `migrate` to be idempotent — safe to run twice — so that
    re-running it on an already-migrated project changes nothing and reports so.
7. As a maintainer, I want `migrate` to refuse to clobber an existing destination
    and tell me why, so that I never silently lose files to a half-done previous run.
7. As a maintainer, I want `migrate` to warn me about the parts it deliberately does
...
7. As a maintainer, I want `migrate` to report a clear summary of what it moved, so
    that I can verify the result at a glance.
7. As the maintainer of vetinari itself, I want to run `migrate` on this repo
...
- **Two directories, fixed meaning (ADR 0001).** `vetinari/` = committed;
  `.vetinari.local/` = excluded. Names chosen to differ by a whole word, not a
  dot.
- **Config candidate precedence.** New order, highest first: `vetinari/config.mts`,
  `vetinari/config.ts`, then the deprecated `.sandcastle/config.mts`. `.sandcastle/config.mts` is
...
- **Extract a pure resolver.** A new `resolveConfigPath(baseDir)` returns the winning
  ...
  `vetinari/config.mts` as the destination.
- **Not-found error lists the canonical path.** When no candidate exists, the error
  leads with `vetinari/config.mts` and mentions `--config <path>`.
- **State default flips.** The default `stateDir` becomes `.vetinari.local` (was
...
- **Migration scope guard.** E1's `migrate` performs the **layout move only**: config →
  `vetinari/`, `.sandcastle/` state+secrets → `.vetinari.local/`, `.gitignore`
...
- **`.gitignore` keeps both entries during the transition.** Add `.vetinari.local/`;
...
- **Modules tested.** (1) `resolveConfigPath` — candidate precedence, a deprecation
  ...
  resolved `stateDir`/`parkedDir`/`logFile` defaults flipping to `.vetinari.local`.
...
- **The `vetinari init` scaffold** for a new project — that is E2 (#13). E1 changes
  where config and state resolve and provides `migrate`; it does not add `init`.
  vetinari is a shared machine install (ADR 0003) — there is no launcher or
...
- Dogfooding this repo (story 22) is the acceptance test for `migrate`: this project's
  own `.sandcastle/` becomes `.vetinari.local/` and a `baseline` still passes.
