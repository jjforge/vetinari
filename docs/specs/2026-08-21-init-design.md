...
vetinari is installed once on my machine and shared by every project, but
standing a *new* project up still means doing the layout by hand: create the
committed `vetinari/` directory, write a config module, copy in a Dockerfile,
create the excluded `.vetinari.local/`, and add it to `.gitignore`. That is
fiddly, easy to get subtly wrong, and something I have to redo for every project I
want to run agents against.

## Solution

A single `vetinari init` command scaffolds a project onto the layout: it creates
a committed `vetinari/` (a config template plus a Dockerfile template to fill in)
and an excluded `.vetinari.local/`, and adds `.vetinari.local/` to `.gitignore`.
Because vetinari is a shared machine install, `init` installs and vendors
...
1. As a maintainer, I want `vetinari init` to create the committed `vetinari/`
   directory, so that my project config has its versioned home without my making it
   by hand.
2. As a maintainer, I want `init` to drop a config template into `vetinari/`, so
   that I start from a working `defineConfig` skeleton instead of a blank file.
3. As a maintainer, I want `init` to drop a Dockerfile template into `vetinari/`,
   so that I have the standard base image to add my toolchain to.
4. As a maintainer, I want `init` to create the excluded `.vetinari.local/`
   directory, so that credentials, logs, and run state have their machine-local home.
5. As a maintainer, I want `init` to add `.vetinari.local/` to `.gitignore` (creating
...
7. As a maintainer, I want `init` to refuse to overwrite an existing `vetinari/`
...
10. As a maintainer, I want `init` not to install or vendor any runtime, so that it
    stays fast and my project never carries a copy of vetinari.
11. As a maintainer, I want the scaffolded config to already point run state at
    `.vetinari.local/` by default, so that the two directories line up out of the box.

## Implementation Decisions

- **Planner + apply.** A pure `computeInit(scan)` takes a description of the target
  directory (does `vetinari/` exist, does `.vetinari.local/` exist, current
...
- **Templates come from the shared install.** The config skeleton and the Dockerfile
  template ship with vetinari (the Dockerfile template already exists in the
  install) and are copied into the project's `vetinari/`. `init` installs nothing.
- **Idempotent and non-clobbering.** Re-running yields an empty plan and a "nothing to
  do" report. An existing `vetinari/config` is never overwritten — `init` refuses
...
- **Modules tested.** `computeInit` — a fresh directory (full scaffold plan), an
  ...
  `vetinari/` config (refusal), and a directory with a `.gitignore` missing the
